...
def get_translation_from_yabla(character):

    pass


with open("hskHanziList.json", "w") as istream:
    json.dump(hsk_voc, istream)


test_text = "子列居鄭圃，四十年人无識者。國君卿大夫眎之，猶眾庶也。國不足，將嫁於衛。弟子曰：「先生往无反期，弟子敢有所謁；先生將何以教？先生不聞壺丘子林之言乎？」子列子笑曰：「壺子何言哉？雖然，夫子嘗語伯昏瞀人，吾側聞之，試以告女。其言曰：有生不生，有化不化。不生者能生生，不化者能化化。生者不能不生，化者不能不化，故常生常化。常生常化者，无時不生，无時不化。陰陽爾，四時爾，不生者疑獨，不化者往復。往復，其際不可終；疑獨，其道不可窮。《黃帝書》曰：「谷神不死，是謂玄牝。玄牝之門，是謂天地之根。綿綿若存，用之不勤。」故生物者不生，化物者不化。自生自化，自形自色，自智自力，自消自息。謂之生化、形色、智力、消息者，非也。"
##test_text = ''
occurencies = Counter(test_text)
del occurencies[" "]
# Frequency in corpus
char_info = {
    w: {
        "frequency": occ / sum(occurencies.values()),
        "hsk level": get_hsk_level(w),
        "translation": get_translation_from_yabla(w),
    }
    for w, occ in occurencies.items()
}
